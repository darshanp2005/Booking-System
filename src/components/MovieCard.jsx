  ...
  return (
    <div className='flex flex-col justify-between p-3 bg-gray-800 rounded-2xl
      ...
      <p className='text-sm text-gray-400 mt-2'>
        ...
        map(genre => genre.name).join(" | ")} - {timeFormat(movie.runtime)} min
      </p>

      <div className='flex items-center justify-between mt-4 pb-3'>
        <button
          onClick={() => {navigate(`/movies/${movie._id}`); scrollTo(0, 0);}}
          className='px-4 py-2 text-xs bg-blue-600 hover:bg-primary-dull transition rounded-full font-medium cursor-pointer'>
          Buy Tickets
        </button>
